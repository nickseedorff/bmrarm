...
fc_sigma_theta_tilde <- function(y, X, prior_precision, y_orig, pat_idx = NULL,
                                 scale_first = FALSE) {
  N_outcome <- ncol(y)
  ## Ignore first 0th observation
  if(is.null(pat_idx)) {
    w_tmp <- y
    X_tilde <- cbind(X, rbind(rep(0, N_outcome), y_orig[-nrow(y), ]))
  } else {
    obs_locs <- data.frame(pat_idx = pat_idx) %>%
      mutate(row_num = row_number()) %>%
      group_by(pat_idx) %>%
      summarise(first_obs = min(row_num),
                last_obs = max(row_num))

    y_tmp <- y_orig
    y_tmp[obs_locs$first_obs, ] <- 0
    y_tmp[-obs_locs$first_obs, ] <- y_orig[-obs_locs$last_obs, ]

    w_tmp <- y
    X_tmp <- X
    X_tilde <- cbind(X_tmp, y_tmp)
  }
...
tmvn_gibbs_rej_alt <- function(y_current, mean, lower, upper, locs, loc_length,
                           pre_calcs, max_iter, N_ord, burn_in, num_iter, num_obs) {
